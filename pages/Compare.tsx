        ...
        <div className={`min-h-screen transition-colors duration-500 antialiased font-sans pt-20 ${theme === 'dark' ? 'bg-background-dark text-slate-200' : 'bg-background-light text-text-light'}`}>
            ...
            <div className="relative z-10">
                <div className="container-custom py-2 md:py-4">
                    <Link to="/products" className={`group inline-flex items-center text-[10px] font-bold uppercase tracking-[0.2em] md:tracking-[0.3em] transition-all ${theme === 'dark' ? 'text-slate-500 hover:text-primary' : 'text-gray-400 hover:text-primary'}`}>
                        <span className="material-icons text-sm mr-1.5 md:mr-2 transition-transform group-hover:-translate-x-1">arrow_back</span>
                        Back to Inventory
                    </Link>
                </div>

                <header className="container-custom py-4 md:py-8 text-center">
                    <span className="inline-block py-1 px-3 md:py-1.5 md:px-4 rounded-full bg-primary/10 border border-primary/20 text-primary text-[9px] md:text-[10px] font-bold tracking-[0.3em] md:tracking-[0.4em] uppercase mb-4 md:mb-6">
                        Technical Comparison
                    </span>
                    <h1 className={`text-fluid-h1 font-display font-bold mb-4 md:mb-6 tracking-tight ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                        Machine Comparison
                    </h1>
                    <p className={`max-w-xl mx-auto text-xs md:text-base font-light mb-8 md:mb-12 px-2 md:px-4 ${theme === 'dark' ? 'text-slate-400' : 'text-gray-500'}`}>
                    ...
                    {availableProducts.length > 0 && (
                        <div className="mb-8 md:mb-12">
                            <h3 className={`text-[10px] font-bold uppercase tracking-[0.3em] text-primary mb-4 md:mb-8 px-2 md:px-4`}>Add Machine to Comparison</h3>
                            <div className="flex flex-wrap justify-center gap-2 md:gap-4 px-2 md:px-4">
                                {availableProducts.map(p => (
                                    <button
                                        key={p.id}
                                        onClick={() => compareIds.length < MAX_MACHINES && addToCompare(p.id)}
                                        disabled={compareIds.length >= MAX_MACHINES}
                                        className={`group relative flex items-center gap-2 md:gap-4 p-2.5 md:p-4 rounded-xl md:rounded-2xl border transition-all hover:scale-105 ${compareIds.length >= MAX_MACHINES ? 'opacity-50 cursor-not-allowed' : ''} ${theme === 'dark' ? 'bg-card-dark/40 border-white/5 hover:border-primary/40' : 'bg-white border-gray-100 shadow-lg md:shadow-xl hover:shadow-primary/10 hover:border-primary/20'}`}
                                    >
                                        <div className={`w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-xl flex items-center justify-center p-1.5 md:p-2 ${theme === 'dark' ? 'bg-black/20' : 'bg-gray-50'}`}>
                                        ...
                                        <div className="w-5 h-5 md:w-8 md:h-8 rounded-full bg-primary/10 flex items-center justify-center group-hover:bg-primary transition-colors">
                                            <span className="material-icons text-primary text-[10px] md:text-xs group-hover:text-white">add</span>
                                        </div>
                                    </button>
                                ))}
                            </div>
                ...
                {selectedProducts.length === 0 ? (
                    <div className="container-custom py-12 md:py-24 text-center">
                        <div className={`inline-flex items-center justify-center w-16 h-16 md:w-24 md:h-24 rounded-2xl md:rounded-[2.5rem] mb-5 md:mb-8 border rotate-45 hover:rotate-0 transition-transform duration-500 ${theme === 'dark' ? 'bg-white/5 border-white/10 text-primary' : 'bg-gray-50 border-gray-100 text-primary shadow-xl shadow-primary/5'}`}>
                            <span className="material-icons text-2xl md:text-4xl -rotate-45">analytics</span>
                        </div>
                        <h2 className={`text-lg md:text-2xl font-display font-bold mb-3 md:mb-4 uppercase ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Comparison Empty</h2>
                        <p className={`mb-6 md:mb-10 text-xs md:text-base max-w-md mx-auto px-2 ${theme === 'dark' ? 'text-slate-500' : 'text-gray-500'}`}>Select up to {MAX_MACHINES} machines from the list above to start the technical comparison.</p>
                        <Link to="/products" className="inline-block bg-primary text-white px-8 py-3 md:px-10 md:py-4 rounded-xl md:rounded-2xl text-[10px] font-bold uppercase tracking-[0.2em] md:tracking-[0.3em] hover:bg-black transition-all shadow-xl shadow-primary/20 active:scale-95">
                            Browse Inventory
                        </Link>
                    </div>
                ) : (
                    <div className="container-custom pb-12 md:pb-24">
                        {/* Control Bar */}
                        <div className="flex justify-between items-center mb-5 md:mb-8 px-1 md:px-0">
                            <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-gray-500">Live Comparison ({selectedProducts.length}/{MAX_MACHINES})</span>
                            <button onClick={clearCompare} className={`flex items-center gap-1.5 md:gap-2 px-4 py-2 md:px-6 md:py-3 rounded-lg md:rounded-xl border text-[9px] md:text-[10px] font-bold uppercase tracking-widest transition-all active:scale-95 ${theme === 'dark' ? 'border-primary/20 text-primary hover:bg-primary/5' : 'border-gray-200 text-gray-500 hover:bg-gray-50 shadow-sm'}`}>
                                <span className="material-icons text-xs md:text-sm">delete_sweep</span>
                                Reset
                            </button>
                        </div>

                        {/* Comparison Table */}
                        <div className={`rounded-2xl md:rounded-[2.5rem] border overflow-hidden backdrop-blur-md transition-all ${theme === 'dark' ? 'bg-card-dark/40 border-white/5' : 'bg-white border-gray-100 shadow-2xl'}`}>
                                ...
                                <table className="w-full border-collapse">
                                    <thead>
                                        <tr className={`${theme === 'dark' ? 'bg-black/60' : 'bg-gray-50'}`}>
                                            <th className={`py-4 px-4 md:py-6 md:px-8 text-left w-1/5 min-w-[140px] md:min-w-[220px] border-r ${theme === 'dark' ? 'bg-black/80 border-white/5' : 'bg-gray-100 border-gray-100'}`}>
                                            ...
                                            {selectedProducts.map(p => (
                                                <th key={p.id} className={`py-4 px-4 md:py-6 md:px-8 text-left min-w-[200px] md:min-w-[280px] border-r last:border-r-0 ${theme === 'dark' ? 'border-white/5 bg-white/5' : 'border-gray-100 bg-white/50'}`}>
                                    ...
                                    <tbody>
                                        ...
                                        {(allDetailKeys as string[]).map((key, idx) => (
                                            <tr key={key} className={`group/row transition-colors ${idx % 2 === 0 ? '' : (theme === 'dark' ? 'bg-white/[0.02]' : 'bg-gray-50/30')}`}>
                                                <td className={`py-3 px-4 md:py-6 md:px-8 border-r text-[10px] md:text-xs uppercase tracking-[0.15em] md:tracking-[0.2em] font-bold border-l-4 border-l-transparent group-hover/row:border-l-primary group-hover/row:bg-primary/5 transition-all ${theme === 'dark' ? 'border-r-white/5 text-gray-400 group-hover/row:text-white' : 'border-r-gray-100 text-gray-500 group-hover/row:text-gray-900'}`}>
                                                    {key.replace(/([A-Z])/g, ' $1').trim()}
                                                </td>
                                                {selectedProducts.map(p => (
                                                    <td key={p.id} className={`py-3 px-4 md:py-6 md:px-8 border-r last:border-r-0 text-xs md:text-base font-bold leading-relaxed transition-all group-hover/row:bg-primary/5 ${theme === 'dark' ? 'border-white/5 text-white' : 'border-gray-100 text-gray-900'}`}>
